...
== Informazioni critiche su Astra Trident 23.07.1
...
----
. Utilizzare `helm install` e specificare un nome per la distribuzione come nell'esempio seguente dove `23.07.1` È la versione di Astra Trident che si sta installando.
+
[listing]
----
...
|===
Ad esempio, per modificare il valore predefinito di `debug`, eseguire quanto segue `--set` comando dove `23.07.1` È la versione di Astra Trident che si sta installando:
